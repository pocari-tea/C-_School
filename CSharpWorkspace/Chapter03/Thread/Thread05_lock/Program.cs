...
namespace Thread05_lock
{
    class NumClass
    {
        ...
        public void IncreaseNum2(object obj)
        {
            ...
            for (int i = 0; i < 10000; i++)
            {
                lock (nc) //nc 객체에 한번에 하나의 스레드만 접근하도록 만든다
                    nc.Number++;
            }
        }
    }
    class Program
    {
        static void Main(string[] args)
        {
            ...
            void unsynchronizeMethod()
            {
                ...
                Console.WriteLine($"unsynchronizeMethod() 실행 결과 : {numObj1.Number}");
            }
            void synchronizeMethod()
            {
                ...
                Console.WriteLine($"unsynchronizeMethod() 실행 결과 : {numObj2.Number}");
            }
        }
    }
}