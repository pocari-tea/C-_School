...
namespace Task08_MakeBreakfast_Async
{
    ...
    class Program
    {
        ...
        {
            var toast = await ToastBreadAsync(number); // 비동기
            ApplyButter(toast);
            ApplyJam(toast);
            return toast;
        }
        ...
        async private static Task<Bacon> FryBaconAsync(int count)
        {
